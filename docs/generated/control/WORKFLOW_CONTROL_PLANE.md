...
- `view_id`: `view.full_control_plane`
- `purpose`: Canonical full process graph view covering local hooks, verify fast runtime path, self-hosting
  diagram subsystem, non-blocking backlog paths, CI jobs, and evidence logging.
- `counts`: actors=3, nodes=37
  artifacts=32, edges=78
- `must_cover_domains`:
  update, generate, validate, lint, test, log, evidence, traceability, hooks, ci, docs_publish
  manual_review, exceptions_non_blocking
...
flowchart TB
  actor_user{{"User"}}
  actor_local_tooling{{"Local Tooling"}}
  actor_ci{{"CI Runner"}}
  ...
  node_validate_workflow_control_plane -->|always / reads| artifact_workflow_control_plane_schema
